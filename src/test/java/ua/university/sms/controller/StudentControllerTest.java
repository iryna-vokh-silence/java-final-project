...
class StudentControllerTest {
    ...
    private StudentResponse sampleResponse() {
        return new StudentResponse(1L, "John", "Doe", "john@uni.ua", 2023, StudentStatus.ACTIVE);
    }

    @Test
    void createStudent_returns201() throws Exception {
        StudentRequest req = new StudentRequest("John", "Doe", "john@uni.ua", 2023, StudentStatus.ACTIVE);
    ...
    void getAll_withStatusParam_callsFilterByStatus() throws Exception {
        when(studentService.filterByStatus(eq(StudentStatus.ACTIVE), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(sampleResponse())));

        mockMvc.perform(get("/api/students").param("status", "ACTIVE"))
                .andExpect(status().isOk())
